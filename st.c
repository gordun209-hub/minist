...
// 65,536 is the number of characters in the original Unicode, and currently in
// a Unicode plane 0x100000.
...
int ttynew(const char *line, char *cmd, const char *out, char **args) {
  ...
  if (out) {
    term.mode or_eq MODE_PRINT;
    // opens term
    iofd = (!strcmp(out, "-")) ? 1 : open(out, O_WRONLY bitor O_CREAT, 0666);
    if (iofd < 0) {
